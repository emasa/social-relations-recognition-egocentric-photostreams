...
class TestNaivePersonDetector:
    def setup_class(self):
        self.detector = NaivePersonDetector()
        self.image_path = os.path.join(ASSETS_DIR, 'american_family.jpg')
    ...
    def test_detect_base(self):
        face = self.faces[0]
        person = self.detector.detect(self.image_path, face)
    ...
    def test_detect_all_base(self):
        image_face_list = zip([self.image_path] * len(self.faces), self.faces)
    ...
    def test_detect_all_one(self):
        people = self.detector.detect_all(self.image_path, self.faces[0])
